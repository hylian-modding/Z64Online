...
export class ModelThread {
    ...
    startThread() {
        ...
        this.child = fork(path.resolve(path.join(__dirname, 'ModelThreadWorker.js')), [filename], options as ForkOptions);
        this.child.on('exit', () => {
            let dest: string = path.join(__dirname, path.parse(filename).name + ".zzcache");
