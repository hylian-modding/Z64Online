export class MatrixTranslate {
    ...
    static FTOFIX32(x: number) {
        let output: number = ((x) * 65536.0);
        return output;
    }

    static guMtxF2L(mf: number[][]): Buffer {
        ...
        for (let i = 0; i < 4; i++) {
            for (let j = 0; j < 2; j++) {
                e1 = MatrixTranslate.FTOFIX32(mf[i][j * 2]);
                e2 = MatrixTranslate.FTOFIX32(mf[i][(j * 2) + 1]);
    ...
    static guRTSF(r: number, p: number, h: number, x: number, y: number, z: number, s: number): number[][] {
